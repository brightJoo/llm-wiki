    ...
    with os.scandir(directory) as entries:
        for entry in sorted(entries, key=lambda item: item.name):
            ...
            if entry.is_dir(follow_symlinks=False):
                ...
                files.extend(_collect(Path(entry.path), child_relative, totals))
            elif entry.is_file(follow_symlinks=False):
                if not _allowed(child_relative):
                    raise SyncError(f"file is not allowed: {child_relative}")
                totals[0] += 1
                totals[1] += entry.stat(follow_symlinks=False).st_size
                if totals[0] > MAX_TOTAL_FILES or totals[1] > MAX_TOTAL_BYTES:
                    raise SyncError("generated Wiki tree exceeds safety limits")
...
def sync_wiki(source: Path, target: Path) -> None:
    ...
    if source_wiki.is_symlink():
        raise SyncError("source docs/wiki must not be a symbolic link")
